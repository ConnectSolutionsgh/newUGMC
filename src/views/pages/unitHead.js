...
const UnitHead = () => {
  ...
  useEffect(() => {
    async function getUnitHeads() {
      const results = await axios.get(
        'https://ugmcservice.herokuapp.com/api/unithead',
      )
      setUnitHeads(results.data)
    }
    getUnitHeads()
  }, [unitHeads])
  ...
  const data = {
    staff: unitHeads.filter((item) =>
      item.head.toLowerCase().includes(search.toLowerCase()),
    ),
  }
  const dataTouse = search.length === 0 ? unitHeads : data.staff
  ...
  const getStaff = async () => {
    const results = await axios.get(
      'https://ugmcservice.herokuapp.com/api/users/getusers',
    )
  ...
  const handleSubmit = async (s) => {
    ...
    if (validate === null) {
      Swal.fire({
        title: 'Are you sure?',
        text: 'Do you want to assign ' + s.fullName + ' as Unit Head ? ',
        icon: 'question',
        ...
        if (result.isConfirmed) {
          try {
            ...
            await axios.put(
              'https://ugmcservice.herokuapp.com/api/users/assignhou/' + s._id,
            )

            if (results.status === 200) {
              Swal.fire(
                'HOU Assigned',
                'Staff Assigned As Head of Unit!',
                'success',
              )
              setStaff({
                staffId: '',
            ...
            getStaff(staff.department, staff.unit)
          } catch (ex) {
      ...
      Swal.fire('HOU Assignment', validate, 'error')
    }
  }
  ...
  const handleRemove = async (s) => {
    Swal.fire({
      title: 'Are you sure?',
      text: 'Do you want to REMOVE ' + s.head + ' as Unit Head ? ',
      icon: 'question',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      ...
      if (result.isConfirmed) {
        const results = await axios.delete(
          'https://ugmcservice.herokuapp.com/api/unithead/removehou/' + s._id,
        )
        if (results.status === 200) {
          await axios.put(
            'https://ugmcservice.herokuapp.com/api/users/removehou/' + s.email,
          )
          Swal.fire('Removed', 'Staff Removed As Head of Unit!', 'success')
        } else {
          return Swal.fire('OOPS', 'Error : Operation Failed', 'error')
        }
      }
          ...
          <CCard>
            ...
            >
              <CModalHeader closeButton>
                <CModalTitle>STAFF MGT | UNIT HEAD </CModalTitle>
              </CModalHeader>
              <CModalBody>
                <form>
                  <CRow>
                    <CCol xs="12" md="12">
                      <CCard>
                        <CCardHeader>SET UNIT HEAD</CCardHeader>
                        <CCardBody>
                          <CFormGroup row>
                            <CCol md="12">
                              <CInputGroup>
            ...
            <CCardBody>
              ...
              <p className="hou-p">Showing {dataTouse.length} Unit Heads...</p>
            </CCardBody>
          </CCard>
        </CCol>
      </CRow>
